...
main(){
	...
	char path[50] = "\\libs\\";//Standard path of the libs
	char file_name[50];

	printf("What tsp data file you want to read ?\n(type the name of file ex:\"kroA100.tsp\")");
	gets(file_name);//Get the  .tsp file name
	path = strcat(path,file_name); //Concat the standard path to the file name
	...
	if(Q != NULL){
		...
		const_matrix_2D(Q);//Build a matrix with the distances between cities
	}else{
